...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('meetings.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Back to Meetings</a>
    ...
    <main class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            <h1 class="text-2xl font-bold text-gray-900 mb-6">Create New Meeting</h1>
            ...
            <form method="POST" action="{{ route('meetings.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="password" id="password"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        placeholder="Leave empty for no password" value="{{ old('password') }}">
                    <p class="mt-1 text-sm text-gray-500">Participants will need this password to join</p>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Leave empty to create an instant meeting</p>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="max_participants" class="block text-sm font-medium text-gray-700 mb-1">Max Participants</label>
                        <input type="number" name="max_participants" id="max_participants" min="2" max="100" value="{{ old('max_participants', 50) }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>
                ...
                <div class="space-y-4 border-t border-gray-200 pt-6">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="waiting_room_enabled" id="waiting_room_enabled" value="1" checked
                            class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                        <label for="waiting_room_enabled" class="ml-2 block text-sm text-gray-700">Enable Waiting Room</label>
                    </div>
                    
                    <div class="flex items-center">
                        <input type="checkbox" name="allow_participants_before_host" id="allow_participants_before_host" value="1"
                            class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                ...
                <div class="flex justify-end space-x-4 border-t border-gray-200 pt-6">
                    <a href="{{ route('meetings.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</a>
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Create Meeting</button>
                </div>
            </form>
        </div>
    </main>
